...
lemma _root_.continuous.strongly_measurable [measurable_space α]
  ...
  {β : Type*} [topological_space β] [pseudo_metrizable_space β] [h : second_countable_topology_either α β]
...
begin
  ...
  { exact measurable_of_tendsto_metric' u (λ i, (hf i).measurable) lim },
...
begin
  borelize β × β,
  have hm : measurable_set {x : β × β | x.1 < x.2},
    from (is_open_lt continuous_fst continuous_snd).measurable_set,
  exact (hf.prod_mk hg).measurable hm
end
