...
lemma measurable_space_const (f : filtration ι m) (i : ι) :
...
lemma sigma_finite_trim_of_le {α} {m₁ m₂ m0 : measurable_space α} {μ : measure α}
  (hm₁₂ : m₁ ≤ m₂) (hm₂ : m₂ ≤ m0)
...
instance sigma_finite_trim [encodable ι] [order_bot ι] {μ : measure α} [sigma_finite_filtration μ f]
  (hτ : is_stopping_time f τ) :
  sigma_finite (μ.trim hτ.measurable_space_le) :=
sigma_finite_trim_of_le (le_measurable_space_of_const_le hτ (λ _, bot_le)) _
...
lemma stopped_value_const (u : ι → α → β) (i : ι) : stopped_value u (λ _, i) = u i := rfl
